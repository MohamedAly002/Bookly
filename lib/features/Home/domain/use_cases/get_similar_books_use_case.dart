import 'package:bookly/config/errors/api_failures.dart';
import 'package:bookly/config/models/book_models/book_model.dart';
import 'package:bookly/features/Home/domain/repo/home_repo_contract.dart';
import 'package:dartz/dartz.dart';
import 'package:injectable/injectable.dart';

@injectable
class GetSimilarBooksUseCase {
  final HomeRepoContract homeRepoContract;

  GetSimilarBooksUseCase(this.homeRepoContract);

  Future<Either<ApiFailures, BooksModel>> call({required String category}) {
    return homeRepoContract.fetchSimilarBooks(category: category);
  }
}
